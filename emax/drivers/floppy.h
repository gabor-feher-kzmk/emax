/* *   		floppy.h includes the header for the floppy drv		  *
   *   -- By Kozmik (C) --                                     _____      *
...
LONG(0x00000001);
/* ----- here are the sig table entrys ----- */
LONG(0x00000010); // this is the num off the signal to handle
LONG(0x00000000); // if it's set the signal need to be masked
LONG(0x00000010); // the PID of the thread that handles the proc 

LONG(0x00000000); // this is the num off the signal to handle
LONG(0x00000000); // if it's set the signal need to be masked
LONG(0x00000000); // the PID of the thread that handles the proc 

LONG(0x00000000); // this is the num off the signal to handle
LONG(0x00000000); // if it's set the signal need to be masked
LONG(0x00000000); // the PID of the thread that handles the proc 
/*=============================== THREAD  [1] ===============================*/
ENTRY(floppy_t1_head);
LONG(0x00000000);//the number of the proc's selector in GDT	
LONG(0x00000000);//LDT selector in GDT 
LONG(0x00000000);//the offset of the LDT
LONG(0x00000000);//the offset of the TSS
...
LONG(0x00000001);//data for priority count		SCHED
LONG(0x00000001);//the status of the proc
/* message data */
LONG(0x00000000);//send stack 
LONG(0xFFFFFFFF);//send dest proc			
LONG(0x00000000);//size of send message			MESSAGE
